...
export default function PageLayout() {
  ...
  useEffect(() => {
    if (pathname.includes('/page/feedback')) setIndex(1);
    else if (pathname.includes('/page/profile')) setIndex(2);
    else if (pathname.includes('/page/request')) setIndex(0);
  }, [pathname]);

  const handleIndexChange = (newIndex: number) => {
    setIndex(newIndex);
    const selected = routes[newIndex].key;
    const validPaths = `/page/${selected}` as '/page/profile' | '/page/request' | '/page/feedback';
